@nomangle GRID_HEIGHT GRID_WIDTH NUM_CAPSULES u_Mouse texture
#version 300 es
precision highp float;
uniform vec2 u_resolution;
uniform vec4 u_Mouse;
uniform vec4 u_MouseMove;
out vec4 fragColor;
...
float sdStickman2(vec2 p, vec2 pos, float headLen, float rot, float super, float magnitude) {
    ...
    vec2 localP = rotate(p - pos, rot + 0.005 * cos(10.0*u_time));
    float time = mod(u_time,2.0*PI);
    ...
    float body = sdCapsuleFixed(localP, bodyPos, bodyLen, 0.0, 10.0 + sin(0.0*max(magnitude, 0.1)*time)*2.0);
...
// Mouse movement is given in u_MouseMovement.x and y, we
// want to calculate the opposite angle and rotate the capsule
float magnitude = u_MouseMove.y;
float rotation = u_MouseMove.x;  

float clicked = u_Mouse.z;
...
float sd = sdStickman2(p, mousePos, 20.0, rotation, min(u_MouseMove.w + clicked, 1.0), magnitude / 30.0);
...
if (u_Mouse.z > 0.0 && u_super > 0.0 && u_superAvailable > 0.0) {
    ...
    circleColor = vec3(u_MouseMove.z); // Default blue color
}
// circleColor = vec3(
...
// float factor = 48.0 / min(u_resolution.x, u_resolution.y);
...
//     finalColor = vec3(0.5 + 0.5*sin(u_time), 0.0, 0.0);
// }
...
if(u_gameStarted < 0.5) {
    ...
    float d3 = sdCapsuleFixed(rotate(p - middle - vec2(-200.0,sin(u_time+PI)*30.0), PI/2.0), vec2(0.0,-22.5), 45.0, PI, 7.0);
    float d4 = digit10(rotate(p - middle - vec2(-200.0,sin(u_time)*30.0), PI/4.0), 30.0);
